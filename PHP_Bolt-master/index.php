...
if(strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') == 0){
	...
	if(strcasecmp($contentType, 'application/json') == 0){
		...
		$hash=hash('sha512', $data->key.'|'.$data->txnid.'|'.$data->amount.'|'.$data->pinfo.'|'.$data->fname.'|'.$data->email.'|||||'."OXYLOANS".'||||||'.$data->salt);
		$json=array();
		$json['success'] = $hash;
...
<div class="main">
    ...
    <div class="dv">
    <span class="text"><label>udf5:</label></span>
    <span><input type="text" id="udf5" name="udf5" placeholder="udf5" value="" /></span>
    </div>
